...
7. AGGREGATE vs. PER-SEGMENT NUMBERS — India CY2025 total EV penetration 8% is dominated by 3W (58%) and 2W (~7%). For 4W PV, EV penetration is ~3% CY2025. NEVER use the all-segment aggregate EV% to score 4W_PV impact as H. Likewise, any market-size figure that aggregates multiple segments (e.g., "₹6.73L Cr auto component market") must NOT be applied as if it describes a single segment. If a factor cites an industry-wide number and is scored H for all segments, it is almost certainly WRONG — downgrade non-primary segments to M or L based on actual per-segment exposure.
