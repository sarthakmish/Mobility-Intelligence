        ...
        <div style={{flex:1,minWidth:0}}>
          <div ref={view===3?svgRef:null} style={{...card(),padding:0,overflow:"hidden",minHeight:"460px",position:"relative"}}>
            <svg width={W} height={H} style={{display:"block"}}>
              ...
              {[0,.25,.5,.75,1].map((f,i)=>{const v=f*mxS;return<g key={i}><line x1={sx(v)} y1={pad.t} x2={sx(v)} y2={H-pad.b} stroke={t.grid} strokeDasharray="2,6"/><text x={sx(v)} y={H-pad.b+14} fill={t.c3} fontSize="8" textAnchor="middle">{fmt(v)}</text></g>})}
