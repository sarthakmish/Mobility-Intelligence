...
{id:"suv",cat:"E",name:"SUV Boom & Premiumization",desc:"UV share 66% of PV. Record 44.89L CY2025 (+5%). Electronics content per SUV 2-3x vs. hatchback.",src:"SIAM CY2025",st:"Accelerating",isNew:false,origin:"2017 — UV overtook cars in volume; structural shift since Creta launch 2015",rel:{"4W_PV":"H","LCV":"L","HCV":"L","2W":"L","3W":"L","Tractor":"L"},pos:{jan25:[8,7.5],jan26:[8.5,8],mar26:[9,8]},pil:["Infotainment","Body & Comfort","ADAS"],segNote:{"4W_PV":"Driving ADAS adoption, premium infotainment (₹15K→₹45K per car), panoramic sunroofs. Electronics content jumps from ₹35K to ₹1.2L per vehicle."}},
{id:"ev",cat:"E",name:"EV Cost Parity Approaching",desc:"EV PV share Apr 2026: 5.8% (vs 3.7% Apr 2025, +57% YoY). Tata leads at 8,543 units Apr 2026. 2W EV ~7% penetration. Battery cost approaching $100/kWh parity.",src:"FADA May 2026 · Vahan",st:"Accelerating",isNew:false,origin:"2019 — first mass-market EVs (Nexon EV, Ather 450); inflection in 2023",rel:{"4W_PV":"H","LCV":"M","HCV":"L","2W":"H","3W":"H","Tractor":"L"},pos:{jan25:[6,8],jan26:[7,8.5],mar26:[7.5,8.5]},pil:["Energy","Motion","Compute","Cloud"],segNote:{"4W_PV":"Battery cost at $100/kWh approaching ICE parity. PV EV share 5.8% Apr 2026 (+75% YoY). Tata 65% share. 4 new EV models in FY26.","2W":"Ola S1 Air at ₹69,999 — cheaper than many ICE scooters. ~7% penetration and rising.","3W":"Fastest EV transition globally. >50% e-3W in Delhi, Bangalore. Battery swap enabling 24/7 operation."}},
{id:"fy26record",cat:"E",name:"India PV FY26 Record: 47L Units",desc:"India PV market closed FY26 at record 47 lakh retail units (+9% YoY) per JATO/SIAM. GST 2.0 + RBI 125bps repo cut to 5.25% + Union Budget 2026 tax relief drove demand. April 2026 = 4.15L units (+11.2% YoY).",src:"JATO May 2026 · SIAM Apr 2026",st:"Active",isNew:true,origin:"Sep 2025 — GST 2.0 + Union Budget 2026 tax relief inflection",rel:{"4W_PV":"H","LCV":"M","HCV":"L","2W":"M","3W":"L","Tractor":"L"},pos:{jan25:[5,6],jan26:[7,7.5],mar26:[8.5,8]},pil:["Motion","Body & Comfort","Infotainment","ADAS"],segNote:{"4W_PV":"Record 47L FY26 vs 43L FY25 = +9% YoY. Auto component industry FY26 estimated ₹7.35L Cr (+9.2%). SUV share 66%, EVs at 5.8% of PV Apr 2026. Sustained tailwind for premium components.","LCV":"LCV growth 7% YoY — last-mile e-commerce expansion. 3W/4W LCV electrification accelerating."}},
...
{n:"Seats & Restraints",p:"Body & Comfort",mat:"Growth",inc:"Seat frames, foam, airbags (4-6), seatbelts, pretensioners",sz:{"4W_PV":23000,"LCV":1300,"HCV":2100,"2W":1600,"3W":320,"Tractor":1200},cagr:12.0,conf:"H",src:"BNCAP FY26"},
...
export default function MobilityIntelligence(){
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <div ref={svgRef} style={{...card(),padding:0,overflow:"hidden",minHeight:"460px",position:"relative"}}>
          <svg width={W} height={H} style={{display:"block"}}>
            ...
            <text x={22} y={H/2} fill={t.c3} fontSize="7.5" textAnchor="middle" transform={`rotate(-90,22,${H/2})`}>Effect on {curr==="EUR"?"€75.6B":"₹6.73L Cr"} auto component market</text>
            ...
            {(()=>{
              ...
              return <g key={b.id} style={{cursor:"pointer"}} onClick={()=>{if(isSel){setV1Sel(null);setAiAnalysis(null);}else{setV1Sel(b);if(b._from_api)fetchPestelAnalysis(b.id);}}} onContextMenu={e=>{e.preventDefault();setV1Compare(p=>p.includes(b.id)?p.filter(x=>x!==b.id):p.length>=3?p:[...p,b.id]);}}>
                ...
                {(()=>{
                  ...
                  // Reference date for NEW-badge eligibility per selected baseline
                  const _refDate=v1Baseline==="now"?new Date():v1Baseline==="jan26"?new Date("2026-01-15"):new Date("2025-01-15");
                  const _originDt=b.originDate?new Date(b.originDate):null;
                  const _ageDays=_originDt?((_refDate-_originDt)/86400000):null;
                  const _isNewAtRef=_ageDays!==null&&_ageDays>=0&&_ageDays<=90;
                  if(v1Baseline==="now"){
                  ...
                  }else if(v1Baseline!=="jan25" && b.st){
                    // qualitative LLM fallback — only when there is a prior baseline to compare against
        ...
        return <div style={{width:"360px",flexShrink:0,overflowY:"auto",maxHeight:"680px",...card()}}>
          ...
          <div style={{padding:"8px",borderRadius:"8px",background:t.btn,marginBottom:"8px"}}>
            ...
            <div style={{fontSize:"11px",color:t.c2,lineHeight:1.55}}>
              Technology market directly affected: <strong style={{color:t.c}}>{fmt(totalMkt)}</strong> ({mktImpactPct}% of {curr==="EUR"?"€75.6B":"₹6.73L Cr"} industry).
  ...
  const renderV3=()=>{
    ...
    // Sort by projected size at v3Year before Top N slice
    const _yrsSort=v3Year-2025;
    data=[...data].sort((a,b)=>{
      const pa=Math.round(a.segSz*Math.pow(1+a.cagr/100,_yrsSort));
      const pb=Math.round(b.segSz*Math.pow(1+b.cagr/100,_yrsSort));
      return pb-pa;
    });
    // When Top N is active with no stage filter, exclude Declining techs from ranking
    if(v3Top>0 && v3Stage==="all")data=data.filter(d=>d.mat!=="Declining");
      ...
      <div style={{display:"flex",gap:"14px",overflow:"hidden"}}>
        <div style={{flex:1,minWidth:0}}>
          <div ref={view===3?svgRef:null} style={{...card(),padding:0,overflow:"hidden",minHeight:"460px",position:"relative"}}>
            <svg width={W} height={H} style={{display:"block"}}>
              ...
              {[0,.25,.5,.75,1].map((f,i)=>{const v=f*mxS;const fmtAxis=v=>{const s=fmt(v);return s.replace(/^[₹€]/,"").replace(/ Cr$/,"");};return<g key={i}><line x1={sx(v)} y1={pad.t} x2={sx(v)} y2={H-pad.b} stroke={t.grid} strokeDasharray="2,6"/><text x={sx(v)} y={H-pad.b+14} fill={t.c3} fontSize="8" textAnchor="middle">{fmtAxis(v)}</text></g>})}
        ...
        {v3T&&(()=>{const d=v3T;const pl=PM[d.p];
          ...
          const mkDrivers=(tech,sg)=>{
            ...
            if(tn.includes("seat")||tn.includes("restraint"))D.push("6-airbag required for 5-star Bharat NCAP — ₹8-12K per vehicle cost uplift; adoption rising in ₹10L+ segment");
